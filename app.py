...
<nav>
<a class="brand" href="{{ url_for('home') }}">JEWEL VOGUE</a>
<a href="{{ url_for('home') }}">Home</a>
<a href="{{ url_for('products') }}">Products</a>
<a href="{{ url_for('contact') }}">Contact</a>
<a href="{{ url_for('track_order') }}">Track Order</a>
<a href="{{ url_for('wishlist') }}">Wishlist ({{ wishlist_count }})</a>
...
def insert_get_id(sql, params=()):
    ...
    try:
        if USE_POSTGRES:
            cur.execute(convert_sql(sql) + ' RETURNING id', params)
            rid = cur.fetchone()[0]
        else:
    ...
    for p in products_data:
        ...
        if not query(
            ...
            insert_get_id(
                ...
                (name,description,price,image,category_id,material_id,stock)
                VALUES(?,?,?,?,?,?,?)
                ''',
                (
                    p[0],
...
def cart_count():
    return sum(to_int(v) for v in get_cart().values())
...
def get_cart_items():
    ...
    for p in rows:
        ...
        out.append({
            'product': p,
            'quantity': q,
            'line_total': q * to_float(p['price'])
        })

    return out


def active_payment_options():

    return [
        x
        for x, k in [
            ('Cash on Delivery', 'cod_enabled'),
            ('Bank Transfer', 'bank_transfer_enabled'),
            ('EasyPaisa', 'easypaisa_enabled'),
            ('JazzCash', 'jazzcash_enabled')
        ]
        if get_setting(k, '0') == '1'
    ]


def active_banks():
    return query(
        'SELECT * FROM banks WHERE active=1 ORDER BY id DESC'
    )
...
def calculate_coupon(code, subtotal):
    ...
    if subtotal < to_float(c['min_amount']):
        return 0, c

    if c['discount_type'] == 'percent':
        d = subtotal * to_float(c['discount_value']) / 100
    else:
        d = to_float(c['discount_value'])
...
'''
    ...
    send_email(
        o['email'],
        f"JEWEL VOGUE - Order #{o['id']} Status: {new_status}",
        body
    )
...
def render_store(body, title='JEWEL VOGUE', **context):
    ...
    context.update(
        ...
        # Admin-controlled contact information
...
def product_card_template():
    ...
    <div class="product">
        ...
        <img src="{{ p.image or 'https://via.placeholder.com/700x500?text=Jewelry' }}">
        ...
        <div class="pad">
            ...
            <div class="actions">
                ...
                <form method="post"
                      action="{{ url_for('cart_add', product_id=p.id) }}">
...
def contact():
    ...
    <div class="box">
        ...
        <div class="actions">
            ...
            <a class="btn"
               href="https://wa.me/{{ contact_number|replace('+','')|replace(' ','')|replace('-','')|replace('(','')|replace(')','') }}"
               target="_blank"
               rel="noopener">

                <span class="social-icon wa">
...
def home():
    ...
    <div class="home-intro">
        ...
        <section class="home-contact">
            ...
            <a class="contact-link whatsapp-link"
               href="https://wa.me/{{ contact_number|replace('+','')|replace(' ','')|replace('-','')|replace('(','')|replace(')','') }}"
               target="_blank"
               rel="noopener">
        ...
        <form class="form-grid">
            ...
            <div>
                ...
                <select name="category">
                    ...
                    <option
                        value="{{ c.id }}"
                        {% if request.args.get('category') == c.id|string %}
                        selected
                        {% endif %}
                    >
                        {{ c.name }}
            ...
            <div>
                ...
                <select name="material">
                    ...
                    <option
                        value="{{ m.id }}"
                        {% if request.args.get('material') == m.id|string %}
                        selected
                        {% endif %}
                    >
                        {{ m.name }}
...
def product_detail(product_id):
    ...
    if not p:
        return render_store(
            '<div class="box"><h2>Product not found</h2></div>',
            'Not Found'
        ), 404
        ...
        <div class="form-grid">
            ...
            <div>
                ...
                <p>
                    ...
                    <a
                        class="btn light"
                        href="{{ url_for('wishlist_toggle', product_id=p.id) }}"
                    >
                        Add/Remove Wishlist
                    </a>
    ...
    <div class="box">
        ...
        <form
            method="post"
            action="{{ url_for('review_add', product_id=p.id) }}"
            class="form-grid"
        >
...
def review_add(product_id):
    ...
    insert_get_id(
        '''
        INSERT INTO reviews
        (product_id,name,rating,comment,approved)
        VALUES(?,?,?,?,1)
        ''',
        (
            product_id,
            request.form.get('name', 'Guest'),
            max(
                1,
                min(
                    5,
...
def cart():
    ...
    discount, coupon = calculate_coupon(
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a
                                class="btn danger"
                                href="{{ url_for('cart_remove', product_id=x.product.id) }}"
                            >
                                Remove
                            </a>
...
def coupon_apply():
    ...
    subtotal = sum(
        x['line_total']
        for x in get_cart_items()
    )

    d, c = calculate_coupon(
        code,
        subtotal
    )
    ...
    if c:
        ...
        session['coupon_code'] = c['code']

        flash(
            f"Coupon applied. Discount: "
        ...
        flash(
            'Invalid coupon or minimum amount not reached.'
        )
...
def checkout():
    ...
    banks = active_banks()
    payments = active_payment_options()
    ...
    if request.method == 'POST':
        ...
        elif method == 'Bank Transfer' and not bank_id:
...
def admin_dashboard():
    ...
    stats = [
        ...
        (
            'Customers',
            query(
                'SELECT COUNT(DISTINCT phone) n FROM orders',
                one=True
            )['n']
        ),
        ...
        <table>
            ...
            <tr>
                ...
                <td class="actions">
                    ...
                    <a
                        class="btn"
                        href="{{{{url_for('{edit_endpoint}',item_id=r.id)}}}}"
                    >
                        Edit
                    </a>
                    ...
                    <a
                        class="btn light"
                        href="{{{{url_for('{toggle_endpoint}',item_id=r.id)}}}}"
                    >
                        Toggle
                    </a>
                    ...
                    <a
                        class="btn danger"
                        href="{{{{url_for('{delete_endpoint}',item_id=r.id)}}}}"
                    >
                        Delete
                    </a>
...
def simple_admin_form(
    ...
    for name, label, typ in fields:
        ...
        if typ == 'textarea':
            ...
            inputs += (
                f'<label>{label}</label>'
                f'<textarea name="{name}">{val}</textarea>'
            )
...
@admin_required
def admin_category_toggle(item_id):
...
@admin_required
def admin_material_toggle(item_id):
        ...
        <table>
            ...
            <tr>
                ...
                <td class="actions">
                    ...
                    <a
                        class="btn"
                        href="{{ url_for('admin_product_edit', item_id=r.id) }}"
                    >
                        Edit
                    </a>
                    ...
                    <a
                        class="btn light"
                        href="{{ url_for('admin_product_toggle', item_id=r.id) }}"
                    >
                        Toggle
                    </a>
                    ...
                    <a
                        class="btn danger"
                        href="{{ url_for('admin_product_delete', item_id=r.id) }}"
                    >
                        Delete
                    </a>
            ...
            <div class="form-grid">
                ...
                <div>
                    ...
                    <select name="category_id">
                        ...
                        <option
                            value="{{ c.id }}"
                            {% if item and item.category_id==c.id %}
                            selected
                            {% endif %}
                        >
                            {{ c.name }}
                ...
                <div>
                    ...
                    <select name="material_id">
                        ...
                        <option
                            value="{{ m.id }}"
                            {% if item and item.material_id==m.id %}
                            selected
                            {% endif %}
                        >
                            {{ m.name }}
                ...
                <div>
                    ...
                    <input
                        type="number"
                        name="active"
                        value="{{ item.active if item else 1 }}"
                    >
...
@admin_required
def admin_product_toggle(item_id):
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <form
                        method="post"
                        action="{{ url_for('admin_order_status', item_id=o.id) }}"
                    >
...
@admin_required
def admin_order_status(item_id):
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a
                        class="btn"
                        href="{{ url_for('admin_review_toggle', item_id=r.id) }}"
                    >
                        Toggle
                    </a>
                    ...
                    <a
                        class="btn danger"
                        href="{{ url_for('admin_review_delete', item_id=r.id) }}"
                    >
                        Delete
                    </a>
...
@admin_required
def admin_review_toggle(item_id):

    execute(
        '''
        UPDATE reviews
        SET approved=CASE approved
        WHEN 1 THEN 0
        ELSE 1
        END
        WHERE id=?
...
@admin_required
def admin_review_delete(item_id):
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a
                        class="btn"
                        href="{{ url_for('admin_coupon_edit', item_id=r.id) }}"
                    >
                        Edit
                    </a>
                    ...
                    <a
                        class="btn light"
                        href="{{ url_for('admin_coupon_toggle', item_id=r.id) }}"
                    >
                        Toggle
                    </a>
                    ...
                    <a
                        class="btn danger"
                        href="{{ url_for('admin_coupon_delete', item_id=r.id) }}"
                    >
                        Delete
                    </a>
    ...
    <div class="box">
        ...
        <form method="post">
            ...
            <select name="discount_type">
                ...
                <option
                    {% if item and item.discount_type=='percent' %}
                    selected
                    {% endif %}
                >
                    percent
                ...
                <option
                    {% if item and item.discount_type=='fixed' %}
                    selected
                    {% endif %}
                >
                    fixed
            ...
            <input
                ...
                step="0.01"
                name="discount_value"
                value="{{ item.discount_value if item else 0 }}"
            >
            ...
            <input
                ...
                step="0.01"
                name="min_amount"
                value="{{ item.min_amount if item else 0 }}"
            >
...
@admin_required
def admin_coupon_toggle(item_id):
...
@admin_required
def admin_coupon_delete(item_id):
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a
                        class="btn"
                        href="{{ url_for('admin_bank_edit', item_id=r.id) }}"
                    >
                        Edit
                    </a>
                    ...
                    <a
                        class="btn light"
                        href="{{ url_for('admin_bank_toggle', item_id=r.id) }}"
                    >
                        Toggle
                    </a>
                    ...
                    <a
                        class="btn danger"
                        href="{{ url_for('admin_bank_delete', item_id=r.id) }}"
                    >
                        Delete
                    </a>
...
def bank_form(item=None):
    ...
    <div class="box">
        ...
        <form method="post">
            ...
            <textarea name="note">{{ item.note if item else '' }}</textarea>
    ...
    if request.method == 'POST':
        ...
        execute(
            ...
            (
                ...
                request.form.get('account_title'),
            ...
            <div class="settings-top">
                ...
                <div class="settings-card">
                    ...
                    <input
                        name="tagline"
                        value="{{ s.tagline }}"
                        placeholder="Elegant jewelry for every occasion."
                    >

                </div>


                <div class="settings-card contact-settings-card">
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(
            'Website: http://127.0.0.1:5051'
        )

        print(
            'Admin: http://127.0.0.1:5051/admin/login'
        )
